...
export function ClaimPassword({ orderId }: { orderId: string }) {
  ...
  return (
    <form onSubmit={submit} className="mx-auto mt-8 max-w-sm rounded-2xl border-2 border-ink bg-lime/40 p-5 text-left">
      ...
      <input type="password" autoComplete="new-password" value={pw} onChange={(e) => setPw(e.target.value)} placeholder="Password (8+ characters)" className="mt-4 h-12 w-full rounded-lg border border-ink/25 bg-white px-4 text-sm outline-none focus:border-ink focus:ring-2 focus:ring-lime" />
